...
(function () {
    ...
    const BUTTON_ID = 'github-sort-by-date-trigger'
    const LABEL_DATE_SORT = '日期排序'
    const LABEL_ORIGINAL_SORT = '原始排序'
    ...
    function updateButtonLabel() {
        ...
        button.textContent = isDateSorted ? LABEL_ORIGINAL_SORT : LABEL_DATE_SORT
        button.title = isDateSorted ? '恢复 GitHub 原始排序' : '按文件更新时间排序'
    }

    function findToolbarRow(goToFileContainer) {
        let node = goToFileContainer?.parentElement
        while (node && node !== document.body) {
            const hasAddFile = node.querySelector('[aria-label="Add file"]')
            const hasCode = node.querySelector('[aria-label="Code"]')
            if (hasAddFile || hasCode) {
                return node
            }
    ...
    function getDirectChild(ancestor, element) {
        let node = element
    ...
    function createButton() {
        ...
        button.id = BUTTON_ID
        button.type = 'button'
        button.textContent = LABEL_DATE_SORT
        button.title = '按文件更新时间排序'
        ...
        // Place the button before "Go to file" within the same toolbar row.
        const goToFile = document.querySelector('[aria-label="Go to file"], [data-hotkey="t"]')
        const goToFileContainer = goToFile?.closest('div.position-relative') || goToFile?.closest('form') || goToFile?.parentElement
        const toolbarRow = findToolbarRow(goToFileContainer)
        const toolbarItem = toolbarRow ? getDirectChild(toolbarRow, goToFileContainer) : null

        if (toolbarRow && toolbarItem) {
            toolbarRow.insertBefore(button, toolbarItem)
        } else if (goToFileContainer?.parentElement) {
            goToFileContainer.insertAdjacentElement('beforebegin', button)
        } else {
            // Fallback to a fixed button so the feature is still reachable.
        ...
        button.addEventListener('click', toggleSortAction)
    }
    ...
    function restoreOriginalSort(files) {
        if (!files || !originalChildren || originalChildren.length === 0) {
            return
        }
        files.replaceChildren(...originalChildren)
    }

    function toggleSortAction() {
        const files = document.querySelector('[aria-labelledby="folders-and-files"] tbody')
        if (!files) {
            return
        }

        if (!isDateSorted) {
            originalChildren = [...files.children]
            sortByDate(files)
            isDateSorted = true
        } else {
            restoreOriginalSort(files)
            isDateSorted = false
    ...
    function initSortButton() {
        if (!document.querySelector('[aria-labelledby="folders-and-files"]')) {
    ...
    window.addEventListener('load', initSortButton)
    document.addEventListener('turbo:load', initSortButton)
})()
